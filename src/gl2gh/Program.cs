...
namespace OctoshiftCLI.GitlabToGithub
{
    public static class Program
    {
        ...
        {
            if (!Path.TrimEndingDirectorySeparator(AppContext.BaseDirectory).EndsWith(Path.Join("extensions", "gh-gl2gh")))
            {
        ...
        private static async Task LatestVersionCheck(ServiceProvider sp)
        {
            ...
            {
                Logger.LogWarning($"You are running an old version of the gl2gh CLI [v{versionChecker.GetCurrentVersion()}]. The latest version is v{await versionChecker.GetLatestVersion()}.");
